...
class ThermostatPopupCard extends LitElement {
    ...
    modeClick(config) {
        if (this.checkModeActive(config))
            return;
        let params = {
        ...
        if (config.temperature) {
            const entity = this.hass.states[config.temperature];
            params = Object.assign(Object.assign({}, params), { temperature: Number(entity.state) });
        }
        if (config.hvac_mode) {
            params = Object.assign(Object.assign({}, params), { hvac_mode: config.hvac_mode });
        }
